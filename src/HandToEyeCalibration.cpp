...
// input
char *extrinsic_param_path = "./extrinsic_param_hand_to_eye.xml";
...
// output (包含外参信息的文件)
char* ex_calib_filepath = "./calibration_ex_params.yml";
...
//    * - 输入1： 多个位姿 -> bTg求逆（转置） -> gTb
    ...
    std::cout << "------------------ 抓手坐标系gripper下基坐标base的表达 bTg -> gTb-----------------" << std::endl;
    vector<Mat> R_base2gripper, t_base2gripper;
...
//    * - 输入2： 多张图片 -> 角点 -> cTt
    ...
    std::cout << "base2gripper: " << R_base2gripper.size() << "-" << t_base2gripper.size() << std::endl;
...
//    * - 输出： 外参矩阵 4x4 -> bTc
    Mat R_camera2base = Mat::eye(3, 3, CV_64F);
    Mat t_camera2base = Mat::zeros(3, 1, CV_64F);
    calibrateHandEye(
            R_base2gripper, t_base2gripper, R_target2camera, t_target2camera,  // 输入
            R_camera2base, t_camera2base, // 输出
            HandEyeCalibrationMethod::CALIB_HAND_EYE_DANIILIDIS
    );
    const Vec3f &euler = rotationMatrixToEulerAngles(R_camera2base);

    std::cout << "旋转矩阵：\n" << R_camera2base << std::endl;
    std::cout << "欧拉角：" << euler *  180.0f / M_PI << std::endl;
    std::cout << "平移矩阵：" << t_camera2base << std::endl;

    // 保存轴角对
    Eigen::Matrix3d eigenRotationMatrix;
    cv::cv2eigen(R_camera2base, eigenRotationMatrix);
    ...
    outFs << "TranslationX" << t_camera2base.at<double>(0,0) * 1000;
    outFs << "TranslationY" << t_camera2base.at<double>(1,0) * 1000;
    outFs << "TranslationZ" << t_camera2base.at<double>(2,0) * 1000;
